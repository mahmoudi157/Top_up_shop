...
public class TransectionHostoryActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle _savedInstanceState) {
		...
		setContentView(R.layout.transection_hostory);
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _v, ViewGroup _container) {
			...
			View _view = _v;
			if (_view == null) {
				_view = _inflater.inflate(R.layout.transection_history, null);
			}
			...
			if (_data.get((int)_position).containsKey("Number")) {
				transection.setText("".concat(_data.get((int)_position).get("Number").toString()));
			}
			if (_data.get((int)_position).containsKey("Method")) {
				method.setText("".concat(_data.get((int)_position).get("Method").toString()));
			}
			if (_data.get((int)_position).containsKey("Amount")) {
				price.setText("৳".concat(_data.get((int)_position).get("Amount").toString().concat(".00")));
			}
			...
			imageview1.setOnClickListener(new View.OnClickListener() {
				@Override
				public void onClick(View _view) {
					((ClipboardManager) getSystemService(getApplicationContext().CLIPBOARD_SERVICE)).setPrimaryClip(ClipData.newPlainText("clipboard", transection.getText().toString()));
